...
module.exports = function(require, debug, options) {
  ...
  var profiler = {
    takeSnapshot: function(name) {
      setTimeout(function() {
        debug.command('HeapProfiler.addHeapSnapshotChunk', { chunk: '' });
        debug.command('HeapProfiler.reportHeapSnapshotProgress', {
          done: 1, total: 2, finished: true
        });
        debug.command('HeapProfiler.reportHeapSnapshotProgress', {
          done: 2, total: 2, finished: true
        });
      }, 10);
    ...
    startTrackingHeapObjects: function() {
      setTimeout(function() {
        debug.command('HeapProfiler.heapStatsUpdate', { statsUpdate: '' });
        debug.command('HeapProfiler.lastSeenObjectId', { lastSeenObjectId: '', timestamp: '' });
      }, 10);
    },
    stopTrackingHeapObjects: function() {},
    getHeapStats: function(iter, cb) { return {}; }
  };
